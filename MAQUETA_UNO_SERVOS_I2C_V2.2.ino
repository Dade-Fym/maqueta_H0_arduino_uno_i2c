...
byte barra[8] =  // creamos barra en sentodo contrario
 ...
 PCF8574 expander1;  //creamos los expander
 PCF8574 expander2;
 PCF8574 expander3;
 PCF8574 expander4;
...
int estadoboton14 = 0;
int estadoanteriorboton14  = 0;
int estadoboton15 = 0;           //guarda el estado del boton
int estadoanteriorboton15  = 0;
int estadoboton16 = 0;
int estadoanteriorboton16  = 0;
...
void setup() {
  ...
  lcd1.home ();                   // go home
   ...
   /* INICIAMOS  I2C bus Y PCF8574 instance */
  expander1.begin(0x20);
  expander2.begin(0x21);
  
  /* CONFIGURACION PINS PCF8574-1 como salidas  */
...
/*
  //CONFIGURACION EXPANDER COMO ENTRADAS
...
*/
 ...
 /* SE DEJAN LIBRES PARA COMUNICACION PC
   pinMode(0, OUTPUT); 
   pinMode(1, OUTPUT);
   pinMode(13, INPUT);*/
       
}
void setServo(uint8_t n_servo, int angulo) {  //cambio a angulo en pwm
  int duty;
...
void loop() {
  ...
  if((estadoboton8 == HIGH)&&(estadoanteriorboton8 == LOW)){  //si estado de boton cambia
    ...
    if(salida8 == 1){
      expander2.digitalWrite(0,HIGH);                    //encendemos el led
      setServo(8,12);                        //mueve el servo para cambiar la aguja
    }
    else{
      expander2.digitalWrite(0,LOW);                       //apaga el led
  ...
  if((estadoboton9 == HIGH)&&(estadoanteriorboton9 == LOW)){  //si estado de boton cambia
    ...
    if(salida9 == 1){
      expander2.digitalWrite(1,HIGH);                    //encendemos el led
      setServo(9,50);   //mueve el servo para cambiar la aguja
    }
    else{
      expander2.digitalWrite(1,LOW);                       //apaga el led
  ...
  if((estadoboton10 == HIGH)&&(estadoanteriorboton10 == LOW)){  //si estado de boton cambia
    ...
    if(salida10 == 1){
     expander2.digitalWrite(2,HIGH);                    //encendemos el led
     setServo(10,10);
    }
    else{
      expander2.digitalWrite(2,LOW);                       //apaga el led
      setServo(10,38);
    }
  ...
  if((estadoboton13 == HIGH)&&(estadoanteriorboton13 == LOW)){  //si estado de boton cambia
    ...
    if(salida13 == 1){
     ...
     estadoboton14 = digitalRead(11);  //lee el estado del boton
  if((estadoboton14 == HIGH)&&(estadoanteriorboton14 == LOW)){  //si estado de boton cambia
    if(antirebote(11)){   //El numero del antirebote debe coincidir con el numero del pin leido digitalread
    salida14 = 1 - salida14; //cambiamos el estado del boton
  }
  }
    estadoanteriorboton14 = estadoboton14;                      //guarda el estado del boton
    ...
    if(salida14 == 1){
     ...
     estadoboton15 = digitalRead(12);  //lee el estado del boton
  if((estadoboton15 == HIGH)&&(estadoanteriorboton15 == LOW)){  //si estado de boton cambia
    if(antirebote(12)){   //El numero del antirebote debe coincidir con el numero del pin leido digitalread
    salida15 = 1 - salida15; //cambiamos el estado del boton
  }
  }
    estadoanteriorboton15 = estadoboton15;                      //guarda el estado del boton
